...
function mapRow(row: Record<string, unknown>): Mentora {
  return {
    ...
    modeloIa: (row.modelo_ia as string) ?? "gpt-4o-mini",
    vectorStoreId: (row.vector_store_id as string | null) ?? null,
    knowledgeFileId: (row.knowledge_file_id as string | null) ?? null,
