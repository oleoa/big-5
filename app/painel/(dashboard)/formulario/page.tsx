...
import { atualizarFormularioAction } from '@/app/painel/actions';
...
export default function FormularioPage() {
  ...
  function adicionar() {
    if (perguntas.length >= 5) return;
    setPerguntas(prev => [
      ...prev,
      { id: '', label: '', tipo: 'text', placeholder: '', obrigatorio: false, ordem: prev.length, falaIa: '' },
    ]);
  }
  ...
  async function handleGuardar() {
    setSaving(true);
    const result = await atualizarFormularioAction(JSON.stringify(perguntas));
    setSaving(false);
    if (result.sucesso) {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Perguntas extras</CardTitle>
          <CardDescription>
            Máximo de 5 campos. Nome, email e celular são sempre solicitados.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-3">
            {perguntas.map((pergunta, i) => (
              <div key={i} className="rounded-lg border p-4 space-y-3">
                ...
                <div className="space-y-1">
                  <Label className="text-xs">Instrução para a IA (fala_ia)</Label>
                  <Textarea
                    value={pergunta.falaIa ?? ''}
                    onChange={e => atualizar(i, 'falaIa', e.target.value)}
                    placeholder="ex: Use a idade do mentorado para contextualizar a análise de personalidade."
                    rows={2}
                    className="text-sm"
                  />
                </div>
          ...
          {perguntas.length === 0 && (
            <p className="text-sm text-muted-foreground mb-3">
              Sem perguntas extras. Nome, email e celular são sempre solicitados.
            </p>
          )}
          ...
          <Button
            ...
            onClick={adicionar}
            disabled={perguntas.length >= 5}
          >
            <Plus className="size-4 mr-2" />
            Adicionar pergunta {perguntas.length >= 5 && '(máximo atingido)'}
          </Button>
